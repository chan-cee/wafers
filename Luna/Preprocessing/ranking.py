...
def get_feature_ranking(ey_test_columns, model, ey_split_data = None, split_column = None): 
    ...
    if split_column:
        ...
        for split in unique_splits_list:
            ...
            metric = tail_weight_ratio + upp_tail_length_ratio + low_tail_length_ratio + outlier_prop
            confidences = rank_pred(model, input_features)

            output_df[f"Split {split} Outlier/Longtail?"] = predictions #pd.Series(predictions, index=header_list)
            output_df[f"Split {split} Model Confidence"] = confidences
            output_df[f"Split {split} Composite Tail Score"] = metric
            output_df[f"Split {split} Tail Weight Ratio"] = tail_weight_ratio
        ...
        output_df["Tail Weight Ratio"] = tail_weight_ratio
